...
The else block can follow the while loop. The else block will be executed when the boolean expression of the while loop evaluates to False. The continue keyword is used to start the next iteration and skip the statements after the continue statement on some conditions.
### for loop 
